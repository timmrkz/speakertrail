...
func (p *Pipeline) ReadEvent(ctx context.Context, eventID, runID int64) error {
	...
	if errors.Is(err, llm.ErrUnreachable) {
		// No model answers. The event stays unread for a later run.
		p.log().Warn("no language model answers, event pages are not read", "error", err)
		return nil
	}
	if err != nil {
		return err
...
func (p *Pipeline) finishRead(ctx context.Context, rec readRecord, people []llm.Person, sourceID int64) error {
	...
	if _, err := tx.Exec(ctx, `
		...
		run, rec.eventID, rec.url, rec.model, rec.found, rec.isNew, rec.err, rec.duration.Milliseconds(), rec.at); err != nil {
		return err
	}
