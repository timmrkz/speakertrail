...
func TestReportShowsFailuresButNoNames(t *testing.T) {
	...
	for _, sql := range []string{
		...
		`INSERT INTO people (full_name, normalised_name, fit, fit_evidence) VALUES ('Lena Musterfrau', 'lena musterfrau', 'founder', 'Lena Musterfrau hat die Backstube Muster gegründet')`,
	...
	for _, want := range []string{
		...
		"Compute error.", "read_event: stopped by hand", "1 manual", "took 00:05:00",
		`from an event page: "… … hat die Backstube Muster gegründet"`, "active: Beispiel Events (https://example.org/events): added", "events waiting for a read: 1", "people with a founder role: 1",
	} {
		if !strings.Contains(out, want) {
			t.Errorf("the report lacks %q:\n%s", want, out)
		}
	}
	if strings.Contains(out, "Musterfrau") || strings.Contains(out, "Lena") {
		t.Errorf("the report names a person:\n%s", out)
	}
}
